...
public class OrderRepositoryTest {
    ...
    void testFindByIdIfIdNotFound() {
        ...
        Order findResult = orderRepository.findById("this-is-not-a-valid-id");
        assertNull(findResult);
    }

    @Test
    void testFindByAuthorIfAuthorCorrect() {
        for (Order order: orders) {
            orderRepository.save(order);
        }

        List<Order> orderList = orderRepository.findByAuthor(
            orders.get(1).getAuthor());
        assertEquals(2, orderList.size());
    }

    @Test
    void testFindByAuthorIfAllLowercase() {
        for (Order order: orders) {
            orderRepository.save(order);
        }
