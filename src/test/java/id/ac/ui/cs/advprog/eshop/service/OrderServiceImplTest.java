...
public class OrderServiceImplTest {
    ...
    void testUpdateStatusInvalidOrderId() {
        doReturn(null).when(orderRepository).findById("another0-0ran-dom0-uuid-tobeusedhere");

        assertThrows(NoSuchElementException.class,
                () -> orderService.updateStatus("another0-0ran-dom0-uuid-tobeusedhere", OrderStatus.SUCCESS.getValue()));
    ...
    void testFindByIdIfIdNotFound() {
        doReturn(null).when(orderRepository).findById("another0-0ran-dom0-uuid-tobeusedhere");
        assertNull(orderService.findById("another0-0ran-dom0-uuid-tobeusedhere"));
    }
    @Test
    void testFindAllByAuthorIfAuthorCorrect() {
        Order order = orders.get(1);
        doReturn(orders).when(orderRepository).findByAuthor(order.getAuthor());
    ...
    void testFindAllByAuthorIfAllLowercase() {
        ...
        doReturn(orders).when(orderRepository)
            .findByAuthor(order.getAuthor().toLowerCase());
